export function IcBaselineStarRate(props) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXlink="http://www.w3.org/1999/xlink"
      width={props.size}
      height={props.size}
      viewBox="0 0 24 24"
      {...props}
    >
      <path
        fill="currentColor"
        d="M14.43 10L12 2l-2.43 8H2l6.18 4.41L5.83 22L12 17.31L18.18 22l-2.35-7.59L22 10z"
      ></path>
    </svg>
  );
}

export function BiBag(props) {
  return (
    <svg width="1.6em" viewBox="0 0 256 256" {...props}>
      <path
        fill="currentColor"
        d="M216 40H40a16 16 0 0 0-16 16v144a16 16 0 0 0 16 16h176a16 16 0 0 0 16-16V56a16 16 0 0 0-16-16Zm0 160H40V56h176v144ZM176 88a48 48 0 0 1-96 0a8 8 0 0 1 16 0a32 32 0 0 0 64 0a8 8 0 0 1 16 0Z"
      ></path>
    </svg>
  );
}

export function PhUserCircle(props) {
  return (
    <svg width="1.6em" viewBox="0 0 256 256" {...props}>
      <path
        fill="currentColor"
        d="M232 128a104 104 0 1 0-174.2 76.7l1.3 1.2a104 104 0 0 0 137.8 0l1.3-1.2A103.7 103.7 0 0 0 232 128Zm-192 0a88 88 0 1 1 153.8 58.4a79.2 79.2 0 0 0-36.1-28.7a48 48 0 1 0-59.4 0a79.2 79.2 0 0 0-36.1 28.7A87.6 87.6 0 0 1 40 128Zm56-8a32 32 0 1 1 32 32a32.1 32.1 0 0 1-32-32Zm-21.9 77.5a64 64 0 0 1 107.8 0a87.8 87.8 0 0 1-107.8 0Z"
      ></path>
    </svg>
  );
}

export function PhUserCirclePlus(props) {
  return (
    <svg width="1.6em" viewBox="0 0 256 256" {...props}>
      <path
        fill="currentColor"
        d="M168 56a8 8 0 0 1 8-8h16V32a8 8 0 0 1 16 0v16h16a8 8 0 0 1 0 16h-16v16a8 8 0 0 1-16 0V64h-16a8 8 0 0 1-8-8Zm62.7 55.6A99.8 99.8 0 0 1 232 128a103.7 103.7 0 0 1-33.8 76.7l-1.3 1.2a104 104 0 0 1-137.8 0l-1.3-1.2A104 104 0 0 1 128 24a99.8 99.8 0 0 1 16.4 1.3a8 8 0 0 1-2.5 15.8A92 92 0 0 0 128 40a88 88 0 0 0-65.8 146.4a79.2 79.2 0 0 1 36.1-28.7a48 48 0 1 1 59.4 0a79.2 79.2 0 0 1 36.1 28.7A87.6 87.6 0 0 0 216 128a92 92 0 0 0-1.1-13.9a8 8 0 0 1 15.8-2.5ZM128 152a32 32 0 1 0-32-32a32.1 32.1 0 0 0 32 32Zm0 64a87.4 87.4 0 0 0 53.9-18.5a64 64 0 0 0-107.8 0A87.4 87.4 0 0 0 128 216Z"
      ></path>
    </svg>
  );
}

export function PhClipboardTextLight(props) {
  return (
    <svg width="1em" height="1em" viewBox="0 0 256 256" {...props}>
      <path
        fill="currentColor"
        d="M166 152a6 6 0 0 1-6 6H96a6 6 0 0 1 0-12h64a6 6 0 0 1 6 6Zm-6-38H96a6 6 0 0 0 0 12h64a6 6 0 0 0 0-12Zm54-66v168a14 14 0 0 1-14 14H56a14 14 0 0 1-14-14V48a14 14 0 0 1 14-14h37.2a45.8 45.8 0 0 1 69.6 0H200a14 14 0 0 1 14 14ZM94 64v2h68v-2a34 34 0 0 0-68 0Zm108-16a2 2 0 0 0-2-2h-29.7a44.9 44.9 0 0 1 3.7 18v8a6 6 0 0 1-6 6H88a6 6 0 0 1-6-6v-8a44.9 44.9 0 0 1 3.7-18H56a2 2 0 0 0-2 2v168a2 2 0 0 0 2 2h144a2 2 0 0 0 2-2Z"
      ></path>
    </svg>
  );
}
